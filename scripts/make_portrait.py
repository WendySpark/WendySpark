...
If the photo isn't already cut out, also `pip install rembg onnxruntime` -
the first run then downloads a ~176 MB background-removal model, once.
A pre-cut PNG (real alpha channel) skips that entirely.
...
def prep(path, crop=None, curve=CURVE, clahe_clip=CLAHE_CLIP):
    """Cut out the background, even the local contrast, then darken.

    If the source already carries real transparency (e.g. pre-cut with a
    background-removal tool), that alpha is used as-is and rembg is never
    imported - the heavy dependency (and its ~176 MB model download) is
    only needed for a flat, un-cut photo.
    """
    src = Image.open(path).convert("RGBA")
    if crop:
        src = src.crop(crop)

    if src.split()[-1].getextrema()[0] < 250:
        cut = src  # already has real transparency
    else:
...
def main():
    ...
    ap.add_argument("--curve", type=float, default=CURVE,
                    help="darkening curve exponent - lower for photos with "
                         "less contrast/darker overall lighting than the "
                         "original reference photo this was tuned on")
    ...
    lines = to_lines(prep(args.photo, crop, curve=args.curve, clahe_clip=args.clahe), cols=args.cols)
    if args.preview:
        print("\n".join(lines))
